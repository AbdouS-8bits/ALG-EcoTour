-- Analytics Database Schema (Standalone Version)
-- Run this SQL to create analytics tables without user/tour references
...
CREATE TABLE IF NOT EXISTS tour_interests (
    ...
    session_id VARCHAR(255) NOT NULL,
    user_id INTEGER, -- No foreign key constraint
    tour_id INTEGER, -- No foreign key constraint
...
CREATE TABLE IF NOT EXISTS user_preferences (
    id SERIAL PRIMARY KEY,
    user_id INTEGER UNIQUE, -- No foreign key constraint
    email VARCHAR(255),
...
CREATE INDEX IF NOT EXISTS idx_tour_interests_user ON tour_interests(user_id);
...
CREATE INDEX IF NOT EXISTS idx_email_engagement_campaign ON email_engagement(campaign_id);
...
CREATE INDEX IF NOT EXISTS idx_conversion_funnels_tour ON conversion_funnels(tour_id);
...
BEGIN
    ...
    RAISE NOTICE 'Tables created: page_views, user_events, user_sessions, tour_interests, search_queries, email_campaigns, email_engagement, user_preferences, conversion_funnels, cookie_consents';
END $$;
